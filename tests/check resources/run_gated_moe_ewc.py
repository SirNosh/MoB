...
# Resource tracking
from resource_utils import ResourceTracker, FlopCounter, count_parameters, format_results
...
def run_experiment(train_tasks, test_tasks, config):
    ...
    for task_id, task_loader in enumerate(train_tasks):
        ...
        train_batch_count += metrics['total_batches']
        train_sample_count += metrics['total_batches'] * config['batch_size']
    ...
    flop_results = FlopCounter.estimate_total_flops(
        model.expert_models[0], sample_input,
        num_train_batches=train_batch_count,
        num_eval_batches=eval_batch_count
    )

    # --- Parameter counts (experts + gater, computed at runtime) ---
    raw_pc = model.count_parameters()
    param_counts_res = {
        'total_params': raw_pc['total_params'],
        'trainable_params': sum(p.numel() for p in model.parameters() if p.requires_grad),
    }

    # --- Format resource results ---
    resource_results = format_results(
        model_name="GatedMoE+EWC",
        tracker_results=tracker_results,
        flop_results=flop_results,
        param_counts=param_counts_res,
    ...
    print(f"  Total Params: {param_counts_res['total_params']:,}")
