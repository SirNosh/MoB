...
class BidLogger:
    ...
    def __init__(self, num_experts: int, log_file: Optional[str] = None):
        """
    ...
    def log_batch(
        ...
        # Log each expert's components
        for expert_id in range(self.num_experts):
            comp = components[expert_id]

            expert_log = {
                'expert_id': expert_id,
                'exec_cost': float(comp['exec_cost']),
                'forget_cost': float(comp['forget_cost']),
                'bid': float(comp['bid']),
                'alpha': float(comp['alpha']),
                'beta': float(comp['beta']),
                'is_winner': (expert_id == winner_id)
            }

            batch_log['experts'].append(expert_log)
        ...
        if self.log_file and batch_idx % 10 == 0:  # Save every 10 batches
    ...
    def save_logs(self, filepath: str):
        ...
        data = {
            ...
            'num_batches': self.stats['num_batches'],
